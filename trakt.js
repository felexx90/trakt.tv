...
export default class Trakt {
  ...
  _exchange(str) {
    ...
    return axios(req).then(response => {
      ...
      throw (error.response && error.response.statusCode === 401) ? Error(
        error.response.headers['www-authenticate']) : error;
    });
  }
  ...
  _device_code(str, type) {
    ...
    this._debug(req);
    return axios(req).
      then(response => this._sanitize(response.data)).
      catch(error => {
        throw (error.response && error.response.statusCode === 401) ? Error(
          error.response.headers['www-authenticate']) : error;
      });
  ...
  _parse(method, params) {
    ...
    for (let k in pathParams) {
      if (pathParams[k][0] !== ':') {
        ...
        if (param || param === 0) {
          ...
          if (method.optional &&
            method.optional.indexOf(pathParams[k].substr(1)) ===
            -1) throw Error(
            `Missing mandatory paramater: ${pathParams[k].substr(1)}`);
        }
      }
    }
    ...
    for (let p in params) {
      filters.indexOf(p) !== -1 &&
      queryParts.indexOf(`${p}=${params[p]}`) === -1 &&
    ...
    if ((params && params.pagination) || this._settings.pagination) {
      ...
      if (method.opts.pagination && response.headers) {
        ...
        let headers = JSON.parse(
          JSON.stringify(response.headers).toLowerCase());
  ...
  _sanitize(input) {
    ...
    const sanitizeObject = obj => {
      ...
      for (let prop in obj) {
        ...
        if (obj[prop] &&
          (obj[prop].constructor === Object || obj[prop].constructor ===
            Array)) {
  ...
  exchange_code(code, state) {
    if (state && state !== this._authentication.state) throw Error(
      'Invalid CSRF (State)');
  ...
  poll_access(poll) {
    if (!poll || (poll && poll.constructor !== Object)) throw Error(
      'Invalid Poll object');
